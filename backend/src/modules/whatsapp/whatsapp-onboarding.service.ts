...
async function exchangeCodeForToken(code: string, redirectUri?: string): Promise<string> {
  ...
  const doFetch = async (useRedirectUri: boolean) => {
    ...
    return { ok: response.ok && !!resData.access_token, resData, status: response.status };
  };

  // For JS SDK popup flows, exchange without redirect_uri first to avoid domain lock
  let attempt = await doFetch(false);
  if (!attempt.ok && redirectUri) {
    attempt = await doFetch(true);
  }

  if (!attempt.ok || !attempt.resData.access_token) {
    const rawMsg =
      attempt.resData?.error?.message || `Token exchange failed (HTTP ${attempt.status}).`;
    console.error('[WhatsappOnboardingService] Token exchange failed from Meta:', {
      status: attempt.status,
      errorCode: attempt.resData?.error?.code,
      errorType: attempt.resData?.error?.type,
      errorMessage: rawMsg,
      fullMetaResponse: attempt.resData,
    });

    let englishMsg = rawMsg;
    if (
      rawMsg.includes('URL लोड') ||
      rawMsg.includes('डोमेन') ||
      rawMsg.toLowerCase().includes("can't load url") ||
      rawMsg.toLowerCase().includes("domain of this url isn't included")
    ) {
      englishMsg =
        "Can't load URL: The domain of this URL is not included in the app's domains. Please add this domain to App Domains in Meta App Settings.";
    } else if (/[\u0900-\u097F]/.test(rawMsg)) {
      englishMsg =
        'Meta OAuth Error: The current domain or redirect URL is not authorized in your Meta App Settings. Please check App Domains in Meta Developer Console.';
    }

    throw new Error(englishMsg);
  }

  console.log('[WhatsappOnboardingService] Token exchange completed successfully.');
  return attempt.resData.access_token;
}
