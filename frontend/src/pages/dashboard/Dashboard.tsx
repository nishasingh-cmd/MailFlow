...
export default function Dashboard() {
  ...
  const activeTab = searchParams.get('tab') === 'whatsapp' ? 'whatsapp' : 'overview';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 border-b border-[var(--surface-border)] pb-3">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all cursor-pointer ${
            activeTab === 'overview'
              ...
              : 'text-[var(--content-secondary)] hover:text-[var(--content-primary)] hover:bg-[var(--surface-elevated)]'
          }`}
        >
          <svg
            className="w-4 h-4"
        ...
        <button
          onClick={() => setSearchParams({ tab: 'whatsapp' })}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all cursor-pointer ${
            activeTab === 'whatsapp'
              ? 'bg-brand-500/10 text-brand-600 dark:text-brand-400 border border-brand-500/30 shadow-xs'
              : 'text-[var(--content-secondary)] hover:text-[var(--content-primary)] hover:bg-[var(--surface-elevated)]'
              ...
              <>
                ...
                <Card variant="elevated" padding="md" className="space-y-3">
                  ...
                  <div className="flex items-baseline justify-between">
                    ...
                    {data?.hasEmailData &&
                      data.emailSuccessRate > 0 &&
                      data.emailSuccessRate < 90 && (
                  ...
                  <p className="text-xs text-[var(--content-tertiary)]">
                    {data?.hasEmailData
                      ? 'Sent / (Sent + Failed) emails'
                      : 'Send emails to see rate'}
                  </p>
                </Card>
              </>
            )}
